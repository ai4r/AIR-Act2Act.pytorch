...
# convert body 3d positions to robot joint angles
def norm_to_joint_angles(body):
    joint_angles = np.array(convert_to_joint_angles(body))
...
# convert robot joint angles to body 3d positions
def denorm_from_joint_angles(features):
    config = configuration()
...
def convert_to_joint_angles(body):
    return joints_to_angles(get_upper_body_joints(body))


def joints_to_angles(joints):
    # joint information
